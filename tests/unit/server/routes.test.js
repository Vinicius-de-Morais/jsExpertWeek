...
describe("#Routes - test site for api respose", () => {
    ...
    test(`GET /controler - should response with ${pages.controllerHTML}  file stream`, async () => {
        const params = TestUtil.defaultHandleParams()
        params.request.method = "GET"
    ...
    test(`POST /unckown - given an inexisten route it should response with 404`, async () => {
        const params = TestUtil.defaultHandleParams()
        params.request.method = "POST"
        params.request.url = "/unckown"
    ...
    describe("exception", () =>{
        test("given an inexisten route it should response with 404", async () => {
            const params = TestUtil.defaultHandleParams()
            params.request.method = "GET"
            ...
            jest.spyOn(
                Controller.prototype,
                Controller.prototype.getFileStream.name, 
            ).mockRejectedValue(new Error('Error: ENOENT: No such file in directory'))
            await handler(...params.values())
