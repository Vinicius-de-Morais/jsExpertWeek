...
export class Service {
  ...
  appendFxStream(fx){
    ...
    const unpipe = () => {
      const transformSteam = this.mergeAudioStream(fx, this.currentReadable)
      this.throttleTransform = throttleTransformable
      this.currentReadable = transformSteam
  ...
  mergeAudioStream(song, readable){
    const transformSteam = PassThrough()
    const args = [
      "-t", audioMediaType,
      "-v", songVolume,
      // -m == merge, o '-' (não é uma carinha) é pra receber como stream
      "-m", '-',
      "-t", audioMediaType,
      "-v", fxVolume,
      song,
    ...
    // plugando a stream de conversacao
    // nos dados do terminal
    streamsPromises.pipeline(
      readable,
      stdin
